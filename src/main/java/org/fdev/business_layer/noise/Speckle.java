...
public class Speckle extends NoiseBaseProcessor {

    private static final String SUCCESS_PROCESS = "Speckle success";
    private static final String FAIL_PROCESS = "Speckle Fail";
    ...
    public ImageFilterResponse filter(String filePath) {
        try{
            if(!filePath.equals("")){
                ...
                //Untuk menampung matrix image* noise
                Mat tempMat = new Mat();

                // Image * noise
                Core.multiply(src , noise , tempMat);
                // Image + (img * noise)
                Core.add(src, tempMat, src);
